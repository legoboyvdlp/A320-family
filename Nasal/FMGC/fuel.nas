...
var updateFuel = func {
	...
	if (fmgc.FMGCInternal.toFromSet and fmgc.FMGCInternal.crzSet and fmgc.FMGCInternal.crzTempSet and fmgc.FMGCInternal.zfwSet) {
		...
		# update climb values
		# fmgc.FMGCInternal.clbFuel;
		# fmgc.FMGCInternal.clbDist = fmgc.FMGCInternal.clbDist * math.max(1.0 - (math.max(pts.Position.gearAglFt.getValue() - 1500.0, 0.0) / crz), 0.0);
		# fmgc.FMGCInternal.clbDist;
		...
		if (num(_desTime) >= 60) {
			...
			fmgc.FMGCInternal.desTime = sprintf("%04d", _desTime);
		}
	...
	for (var i = 0; i <= 2; i += 1) {
		...
		for (var wpt = 0; wpt < fmgc.flightPlanController.arrivalIndex[i]; wpt += 1) {
			...
			if (wpt < fmgc.FMGCInternal.tocIndex[i]) {
				...
				append(time_values[i], _multiplier * fmgc.FMGCInternal.clbTime);
