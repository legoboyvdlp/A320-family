...
var updateFuel = func {
	...
	if (fmgc.FMGCInternal.toFromSet and fmgc.FMGCInternal.crzSet and fmgc.FMGCInternal.crzTempSet and fmgc.FMGCInternal.zfwSet) {
		...
		#todw = 0; # weight after cruise segment
		todw = fmgc.FMGCInternal.tow; # for testing purposes
		...
		if (!fmgc.FMGCInternal.clbReached) {
			...
			for (var i = 0; i <= 2; i += 1) {
				...
				for (var wpt = 1; wpt <= fmgc.flightPlanController.arrivalIndex[i]; wpt += 1) {
					...
					if (toc_distance > fmgc.FMGCInternal.clbDist - fmgc.flightPlanController.traversedDist[i]) {
